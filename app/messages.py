...
WELCOME_TEXT = (
    ...
    f"• `{USER_CMD}` 또는 `{USER_CMD} 설정` — 설정/상태/실행 패널\n"
    f"• `{USER_CMD} 취소` — 진행 중인 회식 흐름 취소\n"
    f"• Slack이 `/{USER_CMD}`을 명령으로 막으면 ` /{USER_CMD}`(앞에 공백) 또는 *설정* 버튼\n"
...
LABEL_WEEKDAY = "요일 (매주·매월 n번째)"
LABEL_DAY = "매월 날짜 (1–28, ‘매월 n일’일 때)"
LABEL_NTH = "번째 주 (매월 n번째, 마지막=-1)"
...
MSG_USE_SETTINGS_BUTTON = "설정 모달은 *설정* 버튼을 누르세요."
...
MSG_POLL_ALREADY_OPEN = (
    ...
    "새 투표가 필요하면 `회식 지금 강제` 또는 관리자 `run-now replace`를 사용하세요."
)
...
MSG_POLL_VOTE_ADDED = "{date} 을(를) 불가능한 날짜로 표시했습니다. 다른 날짜도 선택할 수 있습니다."
MSG_POLL_VOTE_REMOVED = "{date} 불가능 표시를 취소했습니다."
